...
if HAS_PYSIDE:
    class RenderLayerMainWidget(QtWidgets.QWidget):
        ...
        def create_layouts(self):
            ...
            self._cycles_placeholder_ = QtWidgets.QLabel("等待接入...")
            ...
            self._eevee_placeholder_ = QtWidgets.QLabel("等待接入 EEVEE 层...")
            self._eevee_placeholder_.setAlignment(QtCore.Qt.AlignCenter)
            eev_lay.addWidget(self._eevee_placeholder_)
        ...
        def setup_signals(self):
            # 按照您的要求，先剥离复杂的全家桶，踏踏实实“做好一个添加一个”
            # 测试用例 1: 角色Color层 (Cycles)
            self._append_color_layer_pb_.clicked.connect(lambda: self.create_layer_action("Scene_color", 'CYCLES', unique=True))
            # 测试用例 2: Line层 (Eevee)
            self._append_line_layer_pb_.clicked.connect(lambda: self.create_layer_action("Scene_line", 'BLENDER_EEVEE', unique=True))
        ...
        def get_master_scene(self):
            return next((s for s in bpy.data.scenes if s.get("ppas_is_master")), bpy.context.scene)

        def create_layer_action(self, base_name, engine, unique=False):
            master = self.get_master_scene()
            scene_existed = False
            
            if unique and bpy.data.scenes.get(base_name):
                # 场景已经存在，记录状态以便后续只增加渲染层（View Layer）
                scene_existed = True
            ...
            # 找到当前 Scene 中的渲染层最大序号 (如 Scene_color_001)
            max_idx = 0
            ...
            # 如果是首次从母场景拉出该 Scene，清空其自带的冗余旧渲染层
            ...
            # 尝试将它设为当前 UI 激活层
            try:
                bpy.context.window.view_layer = new_vl
            except Exception:
                pass
