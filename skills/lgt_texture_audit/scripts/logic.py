    ...
    class TextureAuditDialog(QtWidgets.QDialog):
        ...
        def run_optimization(self):
            ...
            if self.table.rowCount() == 0:
                ...
                return
            all_paths = []; valid_tasks = []
            for row in range(self.table.rowCount()):
                name_item = self.table.item(row, 1); img_name = name_item.data(QtCore.Qt.UserRole)
                if not img_name: img_name = name_item.text()
                tile_item = self.table.item(row, 0); tile_id_str = tile_item.text() if tile_item else ""
                img = bpy.data.images.get(img_name)
                if img and img.source in ['FILE', 'TILED', 'SEQUENCE'] and img.filepath:
                    try:
                        base_path = os.path.normpath(bpy.path.abspath(img.filepath)); actual_path = base_path
                        if img.source == 'TILED' and tile_id_str: actual_path = self._get_actual_udim_path(base_path, tile_id_str)
                        if os.path.exists(actual_path):
                            all_paths.append(actual_path); valid_tasks.append((row, img, actual_path, base_path))
                        else: print(f"[Warning] Tile file not found on disk: {actual_path}")
                    except: pass
            if not all_paths:
            ...
            if os.path.isfile(common_prefix): common_prefix = os.path.dirname(common_prefix)
            self.prog.setMaximum(len(valid_tasks)); self.prog.setValue(0); processed_count = 0; updated_images = {}
            for step, (row, img, actual_path, base_path) in enumerate(valid_tasks):
                ...
                sugg_res = self.table.item(row, 8).text(); fmt_sugg = self.table.item(row, 10).text()
                rel_path = os.path.relpath(actual_path, common_prefix); new_filepath = os.path.join(target_base_dir, rel_path)
                new_dir = os.path.dirname(new_filepath)
                if not os.path.exists(new_dir): os.makedirs(new_dir, exist_ok=True)
                cmd_args = []; needs_optimization = not (sugg_res == "Keep" and fmt_sugg == "OK")
                ...
                do_process = False
                if not os.path.exists(new_filepath): do_process = True
                else:
                    try:
                        if os.path.getmtime(actual_path) > os.path.getmtime(new_filepath): do_process = True
                    except: do_process = True
                if do_process:
                    if needs_optimization and cmd_args:
                        ...
                        except Exception as e: print(f"Error converting: {e}"); continue
                    else:
                ...
                if os.path.exists(new_filepath):
                    ...
                    if img not in updated_images:
                        rel_base = os.path.relpath(base_path, common_prefix); new_base_filepath = os.path.join(target_base_dir, rel_base)
                        updated_images[img] = new_base_filepath
            for img, new_base_path in updated_images.items(): img.filepath = new_base_path; img.reload()
            QtWidgets.QMessageBox.information(self, "完成", f"处理完成！共处理 {processed_count} 张贴图。路径: {target_base_dir}")

        def add_row(self, name, size_str, mem_mb, score, is_solid, is_grey, has_alpha, is_float, depth, tile_id=""):
            w, h = map(int, size_str.split('x')); mx = max(w, h); target_mem = mem_mb
            ...
            fmt_sugg = []; is_env_format = ("hdr" in name.lower() or "exr" in name.lower()); file_chans = 4
            if is_float:
            ...
            target_chans = float(file_chans)
            if is_grey:
                if file_chans > 1: fmt_sugg.append("转BW")
                target_chans = 1.0
            elif not has_alpha:
                if file_chans == 4 and not is_env_format: fmt_sugg.append("去Alpha")
                target_chans = 3.0
            target_mem = target_mem * (target_chans / 4.0)
            if is_float:
                if "disp" not in name.lower() and not is_env_format:
                    fmt_sugg.append("转8bit"); target_mem = target_mem / 4.0 
            fmt_str = ", ".join(fmt_sugg) if fmt_sugg else "OK"
            if fmt_str == "OK" and sugg_res == "Keep": target_mem = mem_mb
            if fmt_str != "OK":
            ...
            depth_str = "Float" if is_float else "Byte"; self.table.setItem(r, 3, QtWidgets.QTableWidgetItem(depth_str))
            mem_item = QtWidgets.QTableWidgetItem(f"{mem_mb:.1f}"); mem_item.setData(QtCore.Qt.UserRole, mem_mb); self.table.setItem(r, 4, mem_item)
            ...
            self.table.setItem(r, 9, it); self.table.setItem(r, 10, QtWidgets.QTableWidgetItem(fmt_str))
        ...
        def run(self):
            ...
            self.prog.setMaximum(len(imgs)*2); total_mem=0.0; total_opt=0.0; step = 0
            for img in imgs:
                step+=1; self.prog.setValue(step); QtWidgets.QApplication.processEvents()
                is_float = img.is_float; depth = img.depth
                if img.source == 'TILED':
            ...
            self.table.setSortingEnabled(True); self.prog.setValue(self.prog.maximum())
            QtWidgets.QMessageBox.information(self,"完成",f"分析结束。总占用: {gb_val:.2f} GB")

class PPAS_OT_AuditTextures(bpy.types.Operator):
    bl_idname = "ppas.audit_textures"
    bl_label = "Audit Textures"
    def execute(self, context):
        if not HAS_PYSIDE:
            self.report({'ERROR'}, "PySide6 not found")
            return {'CANCELLED'}
        global qt_audit_ref
        app = QtWidgets.QApplication.instance() or QtWidgets.QApplication(sys.argv)
        qt_audit_ref = TextureAuditDialog()
        qt_audit_ref.show()
        return {'FINISHED'}
