...
class PPAS_OT_RefreshProject(bpy.types.Operator):
    bl_idname = "ppas.refresh_project_info"; bl_label = "刷新工程状态"
    def execute(self, context):
        update_project_info(context.scene)
        self.report({'INFO'}, f"工程名已刷新: {context.scene.ppas_project_name}")
        return {'FINISHED'}

classes = (PPAS_OT_RefreshProject,) # 确保包含在 classes 中
